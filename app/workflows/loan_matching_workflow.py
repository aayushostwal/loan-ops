...
async def _calculate_single_match(application_id: int, lender_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        async with WorkflowAsyncSession() as db:
            ...
            await db.execute(
                update(LoanMatch)
                .where(LoanMatch.loan_application_id == application_id, LoanMatch.lender_id == lender_id)
        ...
        logger.error(
            f"Match calculation failed for Application {application_id} " f"and Lender {lender_id}: {str(e)}",
            exc_info=True,
        )
        ...
        try:
            async with WorkflowAsyncSession() as db:
                await db.execute(
                    update(LoanMatch)
                    .where(LoanMatch.loan_application_id == application_id, LoanMatch.lender_id == lender_id)
                    .values(status=MatchStatus.FAILED, error_message=str(e))
                )
                await db.commit()
        except Exception as update_error:
            logger.error(f"Failed to update match status: {str(update_error)}")

        return {"success": False, "application_id": application_id, "lender_id": lender_id, "error": str(e)}


class ProcessApplicationDataInput(BaseModel):
    application_id: int
    raw_text: str
    applicant_name: str
...
if loan_matching_workflow:
    ...
    @loan_matching_workflow.task(execution_timeout=timedelta(minutes=10))
    async def process_application_data(input: ProcessApplicationDataInput, ctx):
        """Step 1: Process raw application data using LLM"""
        logger.info("Step 1: Processing application data using LLM")
        ...
        try:
            ...
            processed_data = await llm_service.process_loan_application(
                raw_text=raw_text, applicant_name=applicant_name or "Unknown"
            )
            logger.info(f"LLM processing successful for application {application_id}")
        except Exception as llm_error:
            logger.error(f"LLM processing failed for application {application_id}: {str(llm_error)}")
            # Continue with error data if LLM fails
            processed_data = {
        ...
        # Update LoanApplication with processed data
        async with WorkflowAsyncSession() as db:
            await db.execute(
            ...
            await db.commit()
            logger.info(f"Updated application {application_id} with processed data")

        return {
            "application_id": application_id,
            "processed_data": processed_data,
            "success": "error" not in processed_data,
        }

    @loan_matching_workflow.task(parents=[process_application_data])
    async def prepare_matching(input, context):
        """Step 2: Prepare matching by fetching lenders and creating match records"""
        logger.info("Step 2: Preparing matching")

        process_result = context.task_output(process_application_data)
        application_id = process_result["application_id"]
        ...
        async with WorkflowAsyncSession() as db:
            ...
            result = await db.execute(select(Lender).where(Lender.status == LenderStatus.COMPLETED))
            lenders = result.scalars().all()

            if not lenders:
                logger.warning("No active lenders found")
                return {"application_id": application_id, "lender_ids": [], "message": "No active lenders available"}

            lender_ids = [lender.id for lender in lenders]

            # Create match records
            for lender_id in lender_ids:
                match = LoanMatch(loan_application_id=application_id, lender_id=lender_id, status=MatchStatus.PENDING)
                db.add(match)
            ...
            return {"application_id": application_id, "lender_ids": lender_ids, "lender_count": len(lender_ids)}

    @loan_matching_workflow.task(parents=[prepare_matching])
    async def calculate_matches(input, context):
        """Step 3: Calculate match scores in parallel for all lenders"""
        logger.info("Step 3: Calculating matches in parallel")

        prepare_result = context.task_output(prepare_matching)
        ...
        if not lender_ids:
            ...
            return {"application_id": application_id, "matches": [], "success_count": 0, "failure_count": 0}

        # Calculate matches in parallel using asyncio
        logger.info(f"Starting parallel match calculation for {len(lender_ids)} lenders")

        tasks = [_calculate_single_match(application_id, lender_id) for lender_id in lender_ids]
        ...
        logger.info(f"Parallel matching completed: {success_count} succeeded, " f"{failure_count} failed")
    ...
    async def finalize_matching(input, context):
        """Step 4: Finalize matching and update application status"""
        logger.info("Step 4: Finalizing matching")

        match_result = context.task_output(calculate_matches)
